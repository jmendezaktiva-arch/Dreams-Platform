...
const COURSES_CONFIG = [
    {
        ...
        description: 'Domina los conceptos a tu propio ritmo con las láminas interactivas y material de apoyo.',
        purposeTitle: 'Tu Ruta Autogestiva',
        purposeDesc: 'Establecer las bases teóricas y el autodiagnóstico inicial para blindar tu operación con autonomía.',
        modality: 'ONLINE',
    ...
    },
    {
        ...
        description: 'Acceso directo al cuaderno de trabajo sincronizado para tu sesión de consultoría vía Zoom.',
        purposeTitle: 'Mentoría Estratégica',
        purposeDesc: 'Acelerar resultados mediante el feedback experto y la resolución de dudas críticas en tiempo real.',
        modality: 'LIVE',
        jsonPath: '/src/academia/courses/consolida-360/sesion-a/presentation/data.json',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderLobby = () => {
        ...
        lobbyContainer.innerHTML = COURSES_CONFIG.map(course => `
            <article class="card" style="${course.isComingSoon ? 'opacity: 0.6; border: 1px dashed #ccc;' : ''}">
                <div class="card-content">
                    <span style="color: ${course.accentColor || '#999'}; font-weight: 600; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px;">
                        ${course.category}
                    </span>
                    <h3 style="${course.isComingSoon ? 'color: #999;' : ''}">${course.title}</h3>
                    <p>${course.description}</p>
                    ${!course.isComingSoon ? `
                        <button class="btn-reveal-purpose" data-action="purpose" data-id="${course.id}">
                            Revelar Propósito
                        </button>
                    ` : ''}
                </div>
                <div class="btn-group">
                    <button class="btn-primary" 
                        ...
                        data-action="open" 
                        data-id="${course.id}"
                        style="font-size: 0.8rem; ${course.id === 'live' ? 'background: #fff; color: var(--secondary-color); border: 2px solid var(--secondary-color);' : ''}">
   ...
   // 1. MOTOR DE RENDERIZADO (LIMPIO)
    const renderSlides = (data) => {
        ...
        // Trazabilidad: Sincronizamos Reveal solo si ya existe, si no, lo iniciamos
    ...
    const toggleBubble = (elementId, forceState = null) => {
        ...
        if (shouldHide) {
            ...
            // Lógica para Video Local: Carga directa al elemento para máxima compatibilidad
            if (elementId === 'video-tutorial') {
                const video = document.getElementById('tutorial-video');
                if (video && !video.src && currentSessionData?.multimedia?.tutorialUrl) {
                    // Asignamos la ruta directamente al video (más confiable que <source>)
                    video.src = encodeURI(currentSessionData.multimedia.tutorialUrl);
                    video.load();
                    video.play().catch(() => console.log("Reproducción automática esperando interacción"));
                    video.volume = 0.4;
                }
            }
        }
    ...
    if (btnPresBack) {
        btnPresBack.addEventListener('click', () => {
            // Cerramos todas las burbujas activas mediante el nuevo motor
   ...
   // --- MOTOR DE PERSISTENCIA UNIVERSAL (DEBOUNCE) ---
        ...
        try {
            await fetch(WEB_APP_URL, {
                ...
                body: JSON.stringify({
                    ...
                    fieldId: fieldId, // Identificamos qué ejercicio se está guardando
                    respuesta: value,
    ...
    let startModule = async (courseId) => {
        ...
        try {
            ...
            // --- MOTOR DE PLAYLIST DINÁMICO (Optimizado para Visibilidad) ---
            if (currentSessionData.multimedia && currentSessionData.multimedia.playlist) {
                const podcastPlayer = document.getElementById('podcast-player');
                const selector = document.getElementById('podcast-selector');
                const podcastSource = document.getElementById('podcast-source');
                const audioElement = document.getElementById('audio-element');
                
                // 1. Mostramos el reproductor primero (Asegura visibilidad inmediata)
                if (podcastPlayer) podcastPlayer.style.display = 'block';

                // 2. Limpiamos y poblamos el selector
                selector.innerHTML = '';
                ...
                // 3. Carga inicial controlada
                if (currentSessionData.multimedia.playlist.length > 0) {
                    const firstTrack = currentSessionData.multimedia.playlist[0];
                    podcastSource.src = encodeURI(firstTrack.url); // Codificamos espacios
                    audioElement.load();
                    audioElement.volume = 0.3;
                }

                // 4. Evento de cambio de pista
                selector.onchange = () => {
                    podcastSource.src = encodeURI(selector.value);
                    audioElement.load();
    ...
    const renderSessions = (courseConfig) => {
        ...
        const sessions = [
            { id: 'a', name: 'Sesión A', topic: 'Cimentación y Propósito', icon: '🏗️' },
            { id: 'b', name: 'Sesión B', topic: 'Estructura y Procesos', icon: '⚙️' },
            { id: 'c', name: 'Sesión C', topic: 'Crecimiento y Escala', icon: '🚀' }
        ];
        ...
        sessionList.innerHTML = sessions.map(session => `
            <article class="card" style="border-top: 4px solid ${courseConfig.accentColor}">
                <div class="card-content" style="text-align: center; padding: 30px 20px;">
                    ...
                    <span style="color: #666; font-size: 0.7rem; font-weight: 700; text-transform: uppercase;">${session.name}</span>
                    <h4 style="margin: 10px 0; color: var(--primary-color); font-size: 1.2rem;">${session.topic}</h4>
                    <button class="btn-primary" 
                        ...
                        style="margin-top: 15px; font-size: 0.8rem; background: ${courseConfig.accentColor}">
                        Acceder a Sesión
                    </button>
                </div>
            </article>
    ...
    window.addEventListener('loadSessionContent', async (e) => {
        const { sessionId, modality, jsonPath } = e.detail;

        try {
            // 1. CÁLCULO DE RUTA: Reemplazamos 'sesion-a' por la carpeta de la sesión elegida (b o c)
            ...
            // 2. CONEXIÓN REAL: Descargamos el JSON específico (Sesión A, B o C)
            const response = await fetch(sessionPath);
            if (!response.ok) throw new Error(`Contenido no hallado en: ${sessionPath}`);
            
            // 3. ACTUALIZACIÓN DE TRAZABILIDAD: Renovamos el cerebro del sistema con los nuevos datos
            currentSessionData = await response.json();
            currentSessionData.courseMetadata.modality = modality;
